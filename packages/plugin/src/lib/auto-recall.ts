...
export function createRecallHandler(pluginConfig) {
    return async (event, ctx) => {
        ...
        if (!agentCreds) return; // Agent not configured for Guild

        let token;
        try {
            token = await getAgentToken(
                { agentId, platformUuid: agentCreds.uuid, email: agentCreds.email, password: agentCreds.password, legacyJwt: agentCreds.jwt },
                pluginConfig
            );
        } catch {
...
async function fetchUserContext(url, headers, senderId, channel) {
    ...
    try {
        const idRes = await fetch(
            `${url}/rest/v1/external_identities?platform=eq.${channel}&platform_user_id=eq.${senderId}&select=user_id,display_name`,
            { headers }
        );
        if (!idRes.ok) return null;
        const ids = await idRes.json();
        if (ids.length === 0) return "[User: new — no prior context]";
        ...
        const memRes = await fetch(
            `${url}/rest/v1/user_memories?user_id=eq.${userId}&select=namespace,key,value&limit=20`,
            { headers }
        );
        if (!memRes.ok) return displayName ? `[User: ${displayName}]` : null;
...
async function fetchRoleContext(url, headers, agentUuid) {
    ...
    try {
        const res = await fetch(
            `${url}/rest/v1/role_memories?status=eq.active&select=namespace,key,value&order=updated_at.desc&limit=10`,
            { headers }
        );
