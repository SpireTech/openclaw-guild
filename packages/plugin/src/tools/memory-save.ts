...
export const memorySaveDef = {
  ...
  description:
    "Save a memory to the OpenClaw Guild. Creates or updates based on (namespace, key). Use namespaces: context, todo, lessons, notes, decisions, observations.",
...
export async function executeMemorySave(
  ...
  return {
    content: [{ type: "text" as const, text: JSON.stringify({ saved: true, memory: result }) }],
    details: undefined,
  };
}
