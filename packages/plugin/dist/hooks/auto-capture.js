 ...
 */
function extractUserFacts(messages) {
    const facts = [];
    const seen = new Set();
 ...
 */
async function saveUserMemory(creds, config, userId, fact) {
    // Check if exists
 ...
 */
async function resolveGrantedUser(creds, config) {
    try {
        const grants = await supabaseGet(creds, config, {
 ...
 */
async function isAutoCaptureDisabled(creds, config, userId) {
    try {
        const users = await supabaseGet(creds, config, {
            table: "users",
 ...
 */
export function createAutoCaptureHook(pluginConfig) {
    return async (event, ctx) => {
        ...
        if (!event.success)
            return;
        console.log(`[guild] agent_end auto-capture: agent="${ctx.agentId}" messages=${messages.length}`);
        try {
