 ...
 */
export function createBootstrapHook(pluginConfig) {
    ...
    return async (_event, ctx) => {
        ...
        try {
            ...
            console.log(`[guild] bootstrap: injected ${content.length} chars for "${agentId}"`);
...
async function buildMemoryDoc(creds, config) {
    const lines = [TOOL_REFERENCE];
    try {
        const memories = await supabaseGet(creds, config, {
 ...
 */
async function buildUserMemoryNote(creds, config) {
    try {
        const memories = await supabaseGet(creds, config, {
            table: "user_memories",
            select: "user_id",
        ...
        return (`## User Memory Transparency\n\n` +
