 ...
 */
function extractWorkingContext(messages) {
 ...
 */
async function saveMemory(creds, config, namespace, key, value, source = "observed", tags = []) {
    // Check if exists
 ...
 */
export function createCompactionFlushHook(pluginConfig) {
    return async (event, ctx) => {
        const creds = resolveAgentCredentials(ctx.agentId, pluginConfig);
        if (!creds)
            return;
        console.log(`[guild] before_compaction: agent="${ctx.agentId}" messages=${event.messageCount} ` +
        ...
        try {
            ...
            await saveMemory(creds, pluginConfig, "context", "pre-compaction-snapshot", `[${timestamp}] Session context before compaction (${event.messageCount} messages):\n• ${context}`, "observed", ["auto-compaction", "context-snapshot"]);
...
//# sourceMappingURL=compaction-flush.js.map