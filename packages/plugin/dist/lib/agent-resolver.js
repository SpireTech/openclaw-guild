 ...
 */
export function resolveAgentCredentials(agentId, pluginConfig) {
    if (!agentId)
        return null;
    // Case-insensitive lookup
    const entry = Object.entries(pluginConfig.agents).find(([id]) => id.toLowerCase() === agentId.toLowerCase());
    if (!entry)
        return null;
    const [, auth] = entry;
