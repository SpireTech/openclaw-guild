...
export const skillSaveDef = {
    ...
    description: "Create a new skill or add a new version to an existing skill. " +
...
export async function executeSkillSave(creds, config, params) {
    ...
    if (!params.name || !params.slug || !params.scope) {
        return {
            content: [{ type: "text", text: "To create a new skill, name, slug, and scope are required." }],
            details: undefined,
        };
    }
    return await createSkill(url, headers, creds, params);
}
async function createSkill(url, headers, creds, params) {
    ...
    if (!skillRes.ok) {
        ...
        return {
            content: [{ type: "text", text: `Failed to create skill: ${skillRes.status} ${body}` }],
            details: undefined,
        };
    }
    ...
    if (!verRes.ok) {
        ...
        return {
            content: [{ type: "text", text: `Skill created but failed to save version: ${verRes.status} ${body}` }],
            details: undefined,
        };
    }
    return {
        content: [{ type: "text", text: `Created skill "${params.name}" (${params.slug}) as draft with version 1.` }],
        details: undefined,
    };
}
async function addVersion(url, headers, creds, config, skillId, content, changeNote) {
    ...
    if (versions.length === 0) {
        return {
            content: [{ type: "text", text: `Skill "${skillId}" not found or has no versions.` }],
            details: undefined,
        };
    }
    ...
    if (!verRes.ok) {
        const body = await verRes.text();
        return {
            content: [{ type: "text", text: `Failed to save version: ${verRes.status} ${body}` }],
            details: undefined,
        };
    }
    // Update skill's current_version
    const patchRes = await fetch(`${url}/rest/v1/skills?id=eq.${skillId}`, {
        method: "PATCH",
        headers: {
            ...headers,
            "Prefer": "return=minimal",
    ...
    if (!patchRes.ok) {
        ...
        return {
            content: [{ type: "text", text: `Version ${nextVersion} saved but failed to update skill pointer: ${patchRes.status} ${body}` }],
            details: undefined,
        };
    }
    return {
        content: [{ type: "text", text: `Added version ${nextVersion} to skill "${skillId}".` }],
        details: undefined,
    };
}
//# sourceMappingURL=skill-save.js.map