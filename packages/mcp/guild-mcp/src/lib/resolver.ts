...
export async function resolveAgentSkills(
  ...
  if (agentError || !agent) throw new Error(`Agent ${agentId} not found`);
  ...
  const extractContent = (s: Record<string, unknown>): string | undefined => {
    if (!includeContent) return undefined;
    const versions = s.skill_versions as Array<Record<string, unknown>> | undefined;
    return versions?.[0]?.content as string | undefined;
  };

  // Agent-level assignments (most specific)
  for (const a of assignments ?? []) {
    const skillId = a.skill_id as string;
    if (!a.is_enabled) {
      disabledSet.add(skillId);
    } else {
      const s = a.skills as Record<string, unknown>;
  ...
  // Role-level skills
  for (const s of (roleSkills ?? []) as Array<Record<string, unknown>>) {
    const id = s.id as string;
    if (!skillMap.has(id) && !disabledSet.has(id)) {
      skillMap.set(id, {
        skill: s as unknown as ResolvedSkill["skill"],
  ...
  for (const s of (companySkills ?? []) as Array<Record<string, unknown>>) {
    const id = s.id as string;
    if (!skillMap.has(id) && !disabledSet.has(id)) {
      skillMap.set(id, {
        skill: s as unknown as ResolvedSkill["skill"],
