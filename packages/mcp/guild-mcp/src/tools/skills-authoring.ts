...
export const createSkillTool = {
  name: "create_skill",
  description:
    "Create a new skill record in draft status with version 1. Admin/owner only based on JWT claims.",
  inputSchema: {
...
export const createSkillVersionTool = {
  ...
  description:
    "Add a new version to an existing skill. Versions are immutable once created. The version number auto-increments.",
  inputSchema: {
    type: "object" as const,
    properties: {
...
export async function handleCreateSkillVersion(
  ...
  if (fetchError) throw new Error(`Skill not found or has no versions: ${fetchError.message}`);
