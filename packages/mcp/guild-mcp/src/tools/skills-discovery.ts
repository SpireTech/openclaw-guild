...
export const listSkillsTool = {
  name: "list_skills",
  description:
    "List available skills, filterable by company_id, scope, status, tags, or keyword search.",
  inputSchema: {
...
export const getSkillTool = {
  name: "get_skill",
  description:
    "Fetch a single skill by ID (or slug+scope) with its current active version content.",
  inputSchema: {
    type: "object" as const,
    properties: {
...
export async function handleListSkills(
  ...
  let query = db
    .from("skills")
    .select(
      "id, name, slug, scope, scope_value, description, current_version, status, tags, metadata",
    );
...
export async function handleGetSkill(
  ...
  if (input.skill_id) {
    ...
    if (input.scope_value) q = q.eq("scope_value", input.scope_value);
    skillQuery = q.single();
  } else {
...
export async function handleGetSkillVersion(
  db: SupabaseClient,
  args: Record<string, unknown> | undefined,
) {
