...
export const resolveSkillsTool = {
  name: "resolve_skills",
  description:
    "Given an agent_id, resolve the effective skill set by walking: agent assignments → role assignments → company defaults. Most specific wins (agent > role > company). Returns merged skill list with sources.",
  inputSchema: {
...
export const assignSkillTool = {
  name: "assign_skill",
  description:
    "Assign a skill to an agent or user. Use is_enabled=false to explicitly disable an inherited skill.",
  inputSchema: {
    type: "object" as const,
    properties: {
...
export async function handleResolveSkills(
  db: SupabaseClient,
