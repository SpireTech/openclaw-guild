...
export async function handleProposePromotion(db, args) {
...
export async function handleListPromotions(db, args) {
...
export async function handleReviewPromotion(db, args) {
    ...
    const { data: promo, error: fetchError } = await db
        ...
        .single();
    if (fetchError || !promo)
        throw new Error("Promotion not found or not pending");
    ...
    const targetTable = promo.target_type === "role_memory" ? "role_memories" : "company_memories";
    const newMemory = {
    ...
    const matchKey = promo.target_type === "role_memory"
...
//# sourceMappingURL=memory-promotions.js.map