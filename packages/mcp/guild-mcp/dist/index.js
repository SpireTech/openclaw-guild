...
const enabledGroups = new Set((process.env.GUILD_TOOLS || "memory,skills,knowledge")
    .split(",")
...
const allTools = [];
const handlers = {};
function registerGroup(tools, handlerMap) {
...
if (enabledGroups.has("knowledge")) {
    ...
    registerGroup([searchKnowledgeTool, getChunkTool, listClientsTool, clientSummaryTool], {
...
// ── MCP Server ──
const server = new Server({ name: "guild-mcp", version: "0.6.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: allTools,
}));
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    const jwt = request.params._meta?.jwt;
    const db = createUserClient(jwt);
    const handler = handlers[request.params.name];
    if (!handler)
        throw new Error(`Unknown tool: ${request.params.name}`);
    return handler(db, request.params.arguments);
});
